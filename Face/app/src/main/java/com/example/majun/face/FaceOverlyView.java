...
/**
 * Created by ws02 on 2016/1/13.
 */
public class FaceOverlyView extends View {
    private Bitmap mBitmap;
    private SparseArray<Face> mFaces;

    public FaceOverlyView(Context context) {
        super(context);
    }

    public FaceOverlyView(Context context, AttributeSet attrs) {
        super(context, attrs);
    }

    public FaceOverlyView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    public void setBitmap(Bitmap bitmap) {
        ...
        if (!detector.isOperational()) {
            Log.i("majun", "mFace 没有赋值");

        } else {
            Frame frame = new Frame.Builder().setBitmap(bitmap).build();
            Log.i("majun", "mFace 赋值");
